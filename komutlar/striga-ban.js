...
if(!["798645974061809675"].some(role => message.member.roles.cache.get(role)) && (!message.member.hasPermission("ADMINISTRATOR"))) 
return message.channel.send(new MessageEmbed().setDescription(`${message.author} Komutu kullanmak için yetkin bulunmamakta.`).setColor('0x800d0d').setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setTimestamp()).then(x => x.delete({timeout: 5000}));
  
const banlog = message.guild.channels.cache.find(c => c.id === '786545189011062823')//Ban log kanalı  
...
message.channel.send(new MessageEmbed().setDescription(`${message.author} tarafından ${kullanici} \`${sebep}\` Sebebiyle Sunucudan Yasaklandı. <a:Ban:814576307845333063>`).setImage("https://cdn.discordapp.com/attachments/797036155085258772/798522251825119272/giphy.gif").setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true})).setColor('0x348f36').setTimestamp()) 
banlog.send(new MessageEmbed().setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true})).setImage("https://media.discordapp.net/attachments/785966740802109460/799696026968784939/standard_6.gif?width=374&height=48").setColor('RANDOM').setTimestamp().setDescription(`<a:incidentag:801089832016805919>**Sunucudan Yasaklandı !**\n<a:incidentag:801089832016805919>**Banlayan Yetkili:** ${message.author} (\`${message.author.id}\`)\n<a:incidentag:801089832016805919>**Banlanan Üye:** ${kullanici.user.tag} (\`${kullanici.user.id}\`)\n<a:incidentag:801089832016805919>**Sebep:** \`${sebep}\` \n <a:incidentag:801089832016805919>**Tarih:** \  ${moment(Date.now()).add(10,"hours").format("HH:mm:ss DD MMMM YYYY")}\` `));
}

exports.conf = {
    aliases: ['yasakla,yargı,yargi'],
    permLevel: 0
  };
